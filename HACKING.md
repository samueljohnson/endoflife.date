...
The site logo is [Openmoji's _hourglass not done_ icon](https://openmoji.org/). Derived icons for various usages, such
as [the web app manifest](/manifest.json) were also generated from Openmoji's SVG icon using
...
Note that `android-chrome-*.png` icons were renamed to `logo-*.png`. Those icons are used in other contexts, such as
on the site as a logo.
